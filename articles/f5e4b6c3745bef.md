...
例えば、『ポケットモンスター 金・銀・クリスタル』では、どくタイプの攻撃がむしタイプに対して効果普通、むしタイプの攻撃がどくタイプに対して効果今ひとつに変更されました。
このような変更も、前述のデータベース設計により、容易に対応できます。

例えば、どくタイプとむしタイプの効果倍率を変更する場合、以下のように `type_relations` テーブルのデータを更新します。

```sql
-- どくタイプの攻撃がむしタイプに対して効果普通に変更
DELETE FROM type_relations
WHERE attacker_type = 'poison' AND defender_type = 'bug';

-- むしタイプの攻撃がどくタイプに対して効果今ひとつに変更
UPDATE type_relations
SET effectiveness_level_id = 'half'
WHERE attacker_type = 'bug' AND defender_type = 'poison';
```

このように、効果倍率の変更もデータの更新だけで実現できるため、システムの柔軟性と保守性が高まります。
